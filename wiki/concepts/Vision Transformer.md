...
它把**图像切分为固定大小的图块(patches)**,每个图块投影为向量后作为 [[Transformer架构|Transformer]] 的输入 token——**完全摒弃 CNN 的卷积结构**,直接用 NLP 的 Transformer 架构处理图像分类。
...
完全套用 [[Transformer架构|Transformer]] 架构,只在输入端做"图像 → token"的转换。
...
ViT 之于视觉,如 [[Transformer架构|Transformer]] 之于 NLP——它使**统一架构**(unified architecture)成为可能:
- 同一架构处理文本、图像、视频、语音
- 共享自注意力、位置编码、训练范式
- 多模态融合天然友好
...
- 直接来源:[[Transformer架构|Transformer]] 架构在视觉领域的应用
